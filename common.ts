...
import { readableStreamToString, DiscourseError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new DiscourseError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new DiscourseError(e, e.response?.data)
                }
                if (innerError instanceof DiscourseError) {
                    // Got "DiscourseError" from the above try block
                    throw innerError;
                }
